...
import { 
  ...
  agentAccounts, 
  userAccounts, 
  productionTestAccounts,
  productionAgentAccounts,
  productionUserAccounts,
  type TestAccount 
} from '@/app/lib/testAccounts';
...
export default function LoginPage() {
  ...
  return (
    <AuthLayout>
      ...
      {/* Konta produkcyjne - widoczne zawsze */}
      <div className="mt-6 border-t border-gray-700 pt-6">
        <div className="text-xs text-gray-400 space-y-2">
          <p className="font-semibold text-gray-300">Konta testowe (kliknij aby wypełnić):</p>
          <div className="space-y-1 font-mono bg-gray-700 p-3 rounded text-[10px]">
            <p className="text-blue-400 font-semibold">AGENCI:</p>
            {productionAgentAccounts.map((account, index) => (
              <p
                key={index}
            ...
            {productionUserAccounts.map((account, index) => (
              <p
                key={index}
                onClick={() => handleAccountClick(account)}
                className="cursor-pointer hover:text-white hover:bg-gray-600 p-1 rounded transition-colors"
      ...
      {/* Konta deweloperskie - tylko w trybie deweloperskim */}
      {isDevelopment && (
        <div className="mt-6 border-t border-gray-700 pt-6">
          <div className="text-xs text-gray-400 space-y-2">
            <p className="font-semibold text-gray-300">Dodatkowe konta deweloperskie (kliknij aby wypełnić):</p>
            <div className="space-y-1 font-mono bg-gray-700 p-3 rounded text-[10px]">
              <p className="text-blue-400 font-semibold">AGENCI:</p>
